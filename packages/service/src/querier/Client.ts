...
export class Client {
  ...
  public async post(
    ...
    if (this._catalog) {
      headers[Headers.CATALOG] = this._catalog;
    }
    if (this._schema) {
      headers[Headers.SCHEMA] = this._schema;
    }
    if (options.session) {
      headers[Headers.SESSION] = options.session;
    }
    if (options.timezone) {
      headers[Headers.TIME_ZONE] = options.timezone;
    }
    if (options.prepares) {
      headers[Headers.PREPARE] = options.prepares
  ...
  private async request(options: {
    ...
    return new Promise((resolve, reject) => {
      ...
      params.headers[Headers.USER] = this._user;
      params.headers[Headers.SOURCE] = this._source;
      params.headers[Headers.USER_AGENT] = this._userAgent;
      if (this._authorization) {
        params.headers[Headers.AUTHORIZATION] = this._authorization;
      }
